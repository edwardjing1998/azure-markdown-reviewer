...
export default function App() {
  ...
  useEffect(() => {
    if (!reviewUserId || !reviewSessionId || !reviewFolder) { setReviewContentPages([]); setReviewDiagnostics(null); setReviewSelected(0); return; }
    setReviewLoading(true); setReviewError("");
    loadReviewFolderContent(reviewUserId, reviewSessionId, reviewFolder)
      .then(result => { setReviewContentPages(result.pages || []); setReviewDiagnostics(result); setReviewSelected(0); })
      .catch(e => { setReviewContentPages([]); setReviewDiagnostics(null); setReviewError(e.message); })
  ...
  return <div className="app">
    ...
    {tab === "uploads" && <main className="upload-browser"><h2>Uploaded Content</h2><p className="hint">Select a user, upload session, and folder to review its generated pages.</p>{reviewError && <p className="error-text">{reviewError}</p>}<div className="selectors"><label>User<select value={reviewUserId} onChange={e => setReviewUserId(e.target.value)}><option value="">Select a user…</option>{reviewUsers.map(user => <option key={user.id} value={user.id}>{user.name} — {user.email} ({user.role})</option>)}</select></label><label>Upload session<select value={reviewSessionId} disabled={!reviewUserId || reviewLoading} onChange={e => setReviewSessionId(e.target.value)}><option value="">Select a session…</option>{reviewSessions.map(session => <option key={session.id} value={session.id}>{session.folderName} — {session.id}</option>)}</select></label><label>Folder<select value={reviewFolder} disabled={!reviewSessionId || reviewLoading} onChange={e => setReviewFolder(e.target.value)}><option value="">Select a folder…</option>{reviewFolders.map(folder => <option key={folder} value={folder}>{folder}</option>)}</select></label></div>{reviewFolder && <div className="selection-summary"><b>Selected folder</b><span>{reviewFolder}</span><small>{reviewPages.length ? `${reviewPages.length} generated page(s) found.` : "No generated pages match this folder yet."}</small></div>}{reviewDiagnostics && <details className="path-debug"><summary>Show path diagnostics</summary><p><b>Selected folder:</b> {reviewDiagnostics.folder}</p><p><b>Normalized folder:</b> {reviewDiagnostics.normalizedFolder}</p><p><b>Uploaded relative paths:</b></p><pre>{(reviewDiagnostics.uploadedRelativePaths || []).join("\n") || "(none)"}</pre><p><b>Matched generated pages:</b></p><pre>{(reviewDiagnostics.pages || []).map(item => item.markdownBlob).join("\n") || "(none)"}</pre><p><b>Generated candidates searched:</b></p><pre>{(reviewDiagnostics.candidates || []).map(item => `${item.id} | sourceBlob=${item.sourceBlob || "(none)"}`).join("\n") || "(none)"}</pre></details>}</main>}
